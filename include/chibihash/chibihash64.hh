...
#if !defined(CHIBIHASH64_HH)
#define CHIBIHASH64_HH
#pragma once
...
namespace chibihash
{
	namespace _detail
	{
		...
		constexpr inline uint64_t load64le(const Byte* p)
		{
			static_assert(sizeof(Byte) == 1, "Invalid byte type");
		...
		template <typename Byte>
		constexpr inline uint64_t chibihash64(const Byte* keyIn, ptrdiff_t len, uint64_t seed)
		{
			static_assert(sizeof(Byte) == 1, "Invalid byte type");
			const Byte* k = keyIn;
			ptrdiff_t l = len;
			...
			for (; l >= 32; l -= 32)
			{
				for (int i = 0; i < 4; ++i, k += 8) {
					uint64_t lane = ::chibihash::_detail::load64le<Byte>(k);
					h[i] ^= lane;
					h[i] *= P1;
			...
			{
				h[i] ^= ::chibihash::_detail::load64le<Byte>(k);
				h[i] *= P2; h[i] ^= h[i] >> 31;
			}
	...
	constexpr inline uint64_t hash64_string(const char* keyIn, uint64_t seed = 0)
	{
		size_t len = 0;
		if (keyIn != nullptr)
		{
		...
		return ::chibihash::_detail::chibihash64<char>(keyIn, len, seed);
	}

	constexpr inline uint64_t hash64_data(const uint8_t* keyIn, ptrdiff_t len, uint64_t seed = 0)
	{
		return ::chibihash::_detail::chibihash64<uint8_t>(keyIn, len, seed);
	}
} // namespace chibihash64

#endif // !defined(CHIBIHASH64_HH)